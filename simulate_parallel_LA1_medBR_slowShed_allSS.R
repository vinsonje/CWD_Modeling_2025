...
print("STARTING RANDOM SHARPSHOOTING SIMULATIONS")
randomSS.output = foreach(z1 = 1:nsim, .errorhandling = "pass", .verbose = TRUE, .options.snow=opts) %dopar% {
  
  setwd("E:/CWD_Modeling/Metapop_Model/Scripts") #for my PC
  source(paste(getwd(), "/CWDSourcer.R", sep = ''))
  setwd("E:/CWD_Modeling/Metapop_Model/Scripts") #for my PC
  source(paste(getwd(), "/CWDparms_100x100_randomSS_medBR_slowShed.R", sep = ''))
  ...
  setwd("E:/CWD_Modeling/Metapop_Model/Simulation_outputs/randomSS_LA1_medBR_slowShed") #for my PC
...
randomSS.output = as.data.frame(do.call(rbind, randomSS.output)) #convert the list to a dataframe

names(randomSS.output) = c("max.pop", "max.pop.time",
...
setwd("E:/CWD_Modeling/Metapop_Model/Simulation_outputs/randomSS_LA1_medBR_slowShed") #for my PC
write.csv(randomSS.output, "randomSS_output_LA1_medBR_slowShed.csv", row.names = FALSE)

# print("STARTING PRIORITY SHARPSHOOTING SIMULATIONS")
# prioritySS.output = foreach(z1 = 1:nsim, .errorhandling = "pass", .verbose = TRUE, .options.snow=opts) %dopar% {
#   
#   setwd("E:/CWD_Modeling/Metapop_Model/Scripts") #for my PC
#   source(paste(getwd(), "/CWDSourcer.R", sep = ''))
#   setwd("E:/CWD_Modeling/Metapop_Model/Scripts") #for my PC
#   source(paste(getwd(), "/CWDparms_100x100_prioritySS_medBR_slowShed.R", sep = ''))
#   
#   start.time = Sys.time()
#   
#   init.model = init.CWD()
...
#   output = extract_metrics(onerun, centroids)
#   
#   setwd("E:/CWD_Modeling/Metapop_Model/Simulation_outputs/prioritySS_LA1_medBR_slowShed") #for my PC
#   
#   file.name = paste("output_", z1, ".csv", sep = "")
#   write.csv(output, file.name, row.names = FALSE)
#   
...
# 
# setwd("E:/CWD_Modeling/Metapop_Model/Simulation_outputs/prioritySS_LA1_medBR_slowShed") #for my PC
# write.csv(prioritySS.output, "prioritySS_output_LA1_medBR_slowShed.csv", row.names = FALSE)
# 
# print("STARTING EDGE SHARPSHOOTING SIMULATIONS")
# edgeSS.output = foreach(z1 = 1:nsim, .errorhandling = "pass", .verbose = TRUE, .options.snow=opts) %dopar% {
#   
#   setwd("E:/CWD_Modeling/Metapop_Model/Scripts") #for my PC
#   source(paste(getwd(), "/CWDSourcer.R", sep = ''))
#   setwd("E:/CWD_Modeling/Metapop_Model/Scripts") #for my PC
#   source(paste(getwd(), "/CWDparms_100x100_edgeSS_medBR_slowShed.R", sep = ''))
#   
#   start.time = Sys.time()
#   
#   init.model = init.CWD()
...
#   output = extract_metrics(onerun, centroids)
#   
#   setwd("E:/CWD_Modeling/Metapop_Model/Simulation_outputs/edgeSS_LA1_medBR_slowShed") #for my PC
#   
#   file.name = paste("output_", z1, ".csv", sep = "")
#   write.csv(output, file.name, row.names = FALSE)
#   
...
# }
# 
# edgeSS.output = as.data.frame(do.call(rbind, edgeSS.output)) #convert the list to a dataframe
# 
# names(edgeSS.output) = c("max.pop", "max.pop.time",
#                          "max.I", "max.I.time", "last.I.time", "max.prev",
#                          "total.I", "max.inc", "time.max.inc",
#                          "max.spread", "max.number.I.cells", "max.spread.area",
#                          "final.avg.prion.density",
...
# 
# setwd("E:/CWD_Modeling/Metapop_Model/Simulation_outputs/edgeSS_LA1_medBR_slowShed") #for my PC
# write.csv(edgeSS.output, "edgeSS_output_LA1_medBR_slowShed.csv", row.names = FALSE)
